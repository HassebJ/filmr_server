...
//        this.api_key.push(key);
//    });
    count = 0;
    apiarr_length = this.api_key.length;
...
function getApi_key(){
    var val = count++ % (apiarr_length);
    if(count === apiarr_length){
        count = 0;
    }
    return val;
...
MovieDB.prototype.requestToken = function(fn){
  var self = this;
    var key = getApi_key();
//    console.log('key' + self.api_key[key]);

  request
  .get(endpoints.base_url  + endpoints.authentication.requestToken)
  .query({'api_key': self.api_key[key]})
...
var execMethod = function(type, params, endpoint, fn){
    var key = getApi_key();
//    console.log('key' + this.api_key[key]);
  params = params || {};
  ...
  var req = request(type, endpoints.base_url + endpoint)
            .query({api_key : this.api_key[key]})
            .set('Accept', 'application/json');
